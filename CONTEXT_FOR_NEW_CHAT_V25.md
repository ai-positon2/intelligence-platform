...
**Social Creative Intelligence Analyst (added 2026-08-25, Phase 1 -- Instagram + YouTube only, agent hidden until Phase 3):** `APIFY_API_TOKEN` (opt, NOT YET SET -- Instagram collection degrades to scrape_failed without it), `YOUTUBE_API_KEY` (opt, NOT YET SET -- YouTube collection returns no videos without it), `SCI_APIFY_INSTAGRAM_ACTOR_ID` (opt, overrides the default `apify/instagram-scraper` actor if it needs swapping). Reuses the platform's existing `ANTHROPIC_API_KEY`/`ANTHROPIC_MODEL` for identification and vision analysis -- no new Anthropic key needed.
