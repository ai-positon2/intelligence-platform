...
import { ExternalLink, Globe, Target, DollarSign, Users, Tag, Lightbulb, TrendingUp, ArrowRight, Search } from 'lucide-react';
...
export function CompetitorsTab({ ads, activeCompetitor, setActiveCompetitor, onNav }: CompetitorsTabProps) {
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        {COMPETITORS.map(c => {
          ...
          const isActive = activeCompetitor === c.domain;
          return (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="p-5">
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                {[['Total Ads', compAds.length, '#0f172a'], ['Active', statusAct, comp.color], ['Formats', Object.keys(fmtCounts).length, '#0f172a']].map(([l,v,col],i) => (
                  <div key={i} className="rounded-xl py-3 px-1" style={{ background:`${comp.color}0a` }}>
                    <p className="text-2xl font-black" style={{ color: col as string }}>{v}</p>
                    <p className="text-[10px] text-slate-400 font-medium">{l}</p>
                  </div>
                ))}
              </div>

              {/* Format pills — each clickable to filter gallery */}
              ...
              <button onClick={() => onNav({ tab: 'gallery', domain: comp.domain, format: 'all', search: '' })}
                      className="mt-3 w-full flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold transition-all hover:opacity-90"
                      style={{ background: `${comp.color}15`, color: comp.color }}>
          ...
          {msgAngles.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              ...
              <ul className="space-y-2">
                {msgAngles.map((a, i) => (
                  <li key={i}
                      onClick={() => onNav({ tab: 'gallery', domain: comp.domain, search: a.split(' ').slice(0,3).join(' ') })}
          ...
          {/* Keywords — click to search gallery */}
          {allKW.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-bold text-slate-800 text-sm">Keywords <span className="text-slate-400 font-normal">({allKW.length})</span></h4>
              ...
              <div className="flex flex-wrap gap-1.5">
                {allKW.slice(0, 40).map((kw, i) => (
                  <button key={i}
                          ...
                          className="text-xs px-2.5 py-1 rounded-full font-medium hover:text-white transition-all"
                          style={{ background: `${comp.color}10`, color: comp.color }}
                          onMouseEnter={e => { (e.target as HTMLElement).style.background = comp.color; (e.target as HTMLElement).style.color = 'white'; }}
                          onMouseLeave={e => { (e.target as HTMLElement).style.background = `${comp.color}10`; (e.target as HTMLElement).style.color = comp.color; }}>
                    {kw}
                  </button>
                ))}
              </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            <div className="flex items-center justify-between mb-4">
              <h4 className="font-bold text-slate-800 text-sm">
                Recent Ads <span className="text-slate-400 font-normal text-xs">({recentAds.length} shown)</span>
              </h4>
              <button onClick={() => onNav({ tab: 'gallery', domain: comp.domain, format: 'all', search: '' })}
                      className="flex items-center gap-1 text-xs font-semibold hover:text-indigo-700 transition-colors"
