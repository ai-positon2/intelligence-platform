...
sci_pipeline's Step 3 already wrote onto each sci_posts row.
...
def _analysis_keywords(analysis: dict | None) -> list[str]:
    """Recurring-theme words from one post's analysis dict -- covers both
...
def _post_keywords(post: dict) -> list[str]:
    """One post's analysis words, only when the analysis actually
    succeeded."""
    if post.get("creative_analysis_status") == "ok":
        return _analysis_keywords(post.get("creative_analysis"))
    return []
