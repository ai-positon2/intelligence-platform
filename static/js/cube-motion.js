/*!
 * Vendored from npm "cube-motion" v0.1.0 (https://www.cube-motion.dev/), MIT, (c) Daniel White.
 * Source: https://cdn.jsdelivr.net/npm/cube-motion@0.1.0/dist/{tokens,dom,rise,leave,morph,reveal}.js
 * Combined into a single file to match this platform's no-build-step static/js/
 * vendoring convention (same pattern as thinking-orbs-engine.js). Logic is
 * unmodified from upstream, with one necessary change: upstream's morph.js and
 * reveal.js each independently declare their own `const style = (el) => el.style`
 * helper, which is fine across separate ES module scopes but a duplicate-
 * declaration SyntaxError once merged into a single shared-scope file -- it is
 * declared once here (next to dom.js's other tiny DOM helpers) and used by both.
 * Everything else is byte-for-byte upstream. Public API is unchanged: rise,
 * leave, morph, reveal (same as upstream's dist/index.js barrel; the React/Vue/
 * Solid/Svelte adapter files are NOT vendored here, vanilla-only).
 * To upgrade: re-fetch each dist file at a newer version, re-merge in this same
 * order (tokens, dom, rise, leave, morph, reveal), re-apply the `style` dedupe,
