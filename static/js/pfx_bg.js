/* Shared WebGL particle background (extracted from Usage dashboard). Loads Three.js from CDN; skips on reduced-motion.
   Runs on every authenticated/marketing page (client_base.html, app_base.html, hub.html, login_preview.html, ...),
   so its per-frame cost is a platform-wide tax, not a one-page cost: capped to devicePixelRatio 1 (was 2, i.e. up to
   4x the fragment work on any Retina/HiDPI screen), antialiasing off (the shader's own soft circular falloff makes
   MSAA invisible here), the O(N) position rewrite + full GPU buffer re-upload now happens every OTHER frame instead
   of every frame, and the loop skips all compute/render work entirely while the tab is hidden. */
...
var rnd;try{rnd=new THREE.WebGLRenderer({canvas:cv,antialias:false,alpha:true});}catch(e){cv.remove();return;}
rnd.setPixelRatio(Math.min(devicePixelRatio,1));rnd.setSize(innerWidth,innerHeight);
...
if(!MOB){var CN=90,K=2;for(var a=0;a<CN;a++){var ai=(a*6+1)%N,bs=[];for(var b=0;b<N;b+=5){if(b===ai)continue;var dx=base[ai*3]-base[b*3],dy=base[ai*3+1]-base[b*3+1],dz=base[ai*3+2]-base[b*3+2];bs.push([dx*dx+dy*dy+dz*dz,b]);}bs.sort(function(p,q){return p[0]-q[0];});for(var k=0;k<K;k++)ed.push([ai,bs[k][1]]);}
...
var t=clk.getElapsedTime();fU.value=t;if(oU)oU.value=t;
fc++;
if(fc%2===0){
...
setTimeout(function(){cv.style.opacity=MOB?'0.28':'0.45';},80);}
