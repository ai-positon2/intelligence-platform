/* thinking-orbs engine v0.3.1 — vendored, unmodified, from the npm package's
   "./engine" subpath export (https://github.com/Jakubantalik/thinking-orbs,
   MIT license, Jakub Antalik). This subpath is deliberately React-free: pure
   geometry + a plain-2D-canvas painter, zero dependencies. Fetched from
   https://cdn.jsdelivr.net/npm/thinking-orbs@0.3.1/dist/engine.es.js and
   committed here so this app never depends on an external CDN at request
   time (this repo has no npm/build step for its Flask-served pages -- see
   static/js/thinking-orb.js for the vanilla-JS component built on top of
   this, a hand-port of the library's own React <ThinkingOrb> component
   logic, which is NOT vendored verbatim since it's React).
   To upgrade: re-fetch the same URL at a newer version, diff, replace. */
function U(n, s, t) {
  return n + (s - n) * t;
}
function nt(n) {
