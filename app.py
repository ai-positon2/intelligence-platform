...
# agent crawls weekly. tracker/slot_checker.py owns the parsing and derivation;
# this app only reads the committed snapshot.
#
# Snapshot rather than a live Sheets read, deliberately: the agent writes to an
# office-internal sheet that cannot be shared with the platform's service
# account, the same wall the Job Change Alert roster hit above. The snapshot is
# regenerated by scripts/import_slot_checker_snapshot.py from an .xlsx export
# and committed. tracker.slot_checker._rows_from_live_sheet documents the one
# function to fill in when that access arrives -- nothing else has to change.
