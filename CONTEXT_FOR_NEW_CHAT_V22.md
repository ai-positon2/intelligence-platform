...
- **Admins (`ADMIN_EMAILS`, app.py ~line 1222):** `krishna.ladha@position2.com`, `sudheer.d@position2.com`, `reporting@position2.com`, `sparikh@position2.com`, `abhilash.dg@position2.com`, `pushpendra.k@position2.com`. This set is the ONLY place admin access is defined: `admin_required` gates every `/p2/admin/*` route off it, the template context processor derives `is_admin` from it (drives the shared `_admin_menu.html` dropdown), and `/api/whoami` now returns `is_admin` from it too, so client-rendered surfaces (the legacy signal dashboards, the Ad Intelligence bundle) read the same flag. Add a person here and nowhere else.
