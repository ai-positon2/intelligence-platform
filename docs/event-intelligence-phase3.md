...
## Fifth increment: destination reads and version-bound review

New lookup runs inspect up to four unique organizer access destinations discovered during successful harvesting. The worker checkpoints this read stage; no additional model or Apollo calls occur. Requests use the existing hardened public-page fetcher. Failed reads and redirects outside the organizer host retain explicit failure records, with outside-host content excluded. These attempts are recorded as `access_review` sources, so source coverage/unreadable counts now include access-page reads as well as roster reads.

The plan shows literal price, deadline, eligibility and availability lines, preserving qualifiers and source snapshots. Up to five lines per field and 500 characters per line are retained. Matching text is not normalized into a ticket price, deadline or availability assertion; the page can describe a historical edition, another ticket tier or exceptions. All visible non-copyright years are exposed for review, with edition ownership still unresolved. The four-destination limit, omitted candidates, truncation and missing terms remain explicit. Existing reports are not backfilled.

An offline Phase 3 review packet/evaluator binds human judgments to exact captured plan/evidence fingerprints. It requires two distinct named reviewers and Boolean affirmative labels for applicable checks. Missing/negative judgments, changed snapshots, duplicate cases and unsaved plans cannot pass. Reviewer identity is declared, not authenticated by this offline tool. Even a passing case review returns `release_accepted: false`; it does not replace the live benchmark or prove general recommendation quality.

Remaining acceptance and research work: independently reviewed Position2 outputs; real cost/latency/coverage measurements; verified and conflicting edition-fact resolution; genuinely calibrated action selection; authenticated staging and release operations. The repository now has bounded evidence aids and review gates, not an autonomous booking or verified-fact system. CRM, delivery and monitoring integrations remain separately scoped.
