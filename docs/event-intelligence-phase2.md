...
1. Run migrations and tests against disposable native PostgreSQL matching production. Exercise two workers, simultaneous same-key submissions, concurrent budget reservations, cancellation during a write, connection loss, rollback, and process kills before/after provider responses and stage saves.
2. Validate the pinned environment on Linux and deploy matching web/worker services to staging. Verify queue visibility, cancellation, restart behavior, stored reports and page/API/CSV parity with an authenticated account.
